...
float hudAlpha(IClientInstance& client) {
    // Native custom HUD rendering does not use the ordinary UIControl alpha
    // ancestry cache (it yielded zero for visibly rendered icons in both layouts).
    // Invocation + mShouldRender is the native visibility gate; use the HUD option.
    float alpha = client.getOptions().getInterfaceOpacity();
    return std::isfinite(alpha) ? std::clamp(alpha, 0.0f, 1.0f) : 0;
}
...
void diagnostics(char const* kind, std::span<glm::vec3 const> positions, MinecraftUIRenderContext& context,
    ...
    if (config.diagnostics && logger) {
        ...
        if (now - last >= std::chrono::seconds(2)) {
            ...
            logger->info("HUD {} pass={} owner=({},{}) first=({},{},{}) second=({},{},{}) count={} alpha={} thread={}",
                         kind, pass, anchor.x, anchor.y, first.x, first.y, first.z, second.x, second.y, second.z,
                         positions.size(), hudAlpha(client), GetCurrentThreadId());
    ...
    if (config.diagnosticMarkers && state && pass == config.renderPass && hudAlpha(client) > 0) {
...
void Runtime::hunger(HudHungerRenderer& renderer, MinecraftUIRenderContext& context,
    ...
    diagnostics("hunger", positions, context, client, owner, pass, state ? &*state : nullptr, config, logger);
    if (!state || pass != config.renderPass || !renderer.mShouldRender || !renderer.mHasLoadedTextures) return;
    float alpha = hudAlpha(client);
    if (alpha <= 0) return;
...
void Runtime::hearts(HudHeartRenderer& renderer, MinecraftUIRenderContext& context,
    ...
    auto const& positions = renderer.mIconPosition.get();
    if (!renderer.mShouldRender) return;
    diagnostics("heart", positions, context, client, owner, pass, state ? &*state : nullptr, config, logger);
    if (!state || !config.showRegeneration || pass != config.renderPass || !renderer.mShouldRender
    ...
    float alpha = hudAlpha(client) * pulseAlpha(config);
